...
namespace SBaier.Master
{
	...
	public class SimplexNoise : Noise3D, Seedbased
	{
		...
		public static readonly double F2 = 0.5 * (Math.Sqrt(3.0) - 1.0);
		public static readonly double G2 = (3.0 - Math.Sqrt(3.0)) / 6.0;
		...
		public double[] Evaluate(Vector3[] points)
		{
			...
			IndexableNativeArray<short> dP = new IndexableNativeArray<short>(_dP.ToArray(), Allocator.TempJob);
			IndexableNativeArray<short> dPMod = new IndexableNativeArray<short>(_dPMod.ToArray(), Allocator.TempJob);

			SimplexNoise3DEvaluationJob job = new SimplexNoise3DEvaluationJob(evaluatedPoints, p, dP, dPMod);
		...
		public double[] Evaluate(Vector2[] points)
		{
			...
			SimplexNoise2DEvaluationJob job = new SimplexNoise2DEvaluationJob(evaluatedPoints, p, dP, dPMod);
